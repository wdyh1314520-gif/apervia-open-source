...
1. Update the root `VERSION` file. The current formal version is `1.0.3`.
2. Move `[Unreleased]` content into `## [1.0.3] - 2026-07-25`.
3. Update both release announcement files; their IDs must be `v1.0.3` and their versions must be `1.0.3`.
...
git switch main
git pull --ff-only origin main
git tag -a v1.0.3 -m "Apervia 1.0.3"
...
ghcr.io/wdyh1314520-gif/apervia-open-source:1.0.3
...
ghcr.io/wdyh1314520-gif/apervia-open-source-sandbox:1.0.3
...
```bash
gh release create v1.0.3 --title "Apervia 1.0.3" --notes-file release/announcement.md
```
...
- **Announcement ID**: `v` plus `VERSION`; currently `v1.0.3`.
- **Version**: Exactly match the root `VERSION`; currently `1.0.3`.
...
```dotenv
APP_IMAGE=ghcr.io/wdyh1314520-gif/apervia-open-source:1.0.3
SANDBOX_DOCKER_IMAGE=ghcr.io/wdyh1314520-gif/apervia-open-source-sandbox:1.0.3
```
...
Create a dedicated branch from the latest `main` even for an emergency. Include only the minimum fix and its regression test. After merging, verify the `sha-<commit>` image before deciding whether to publish a patch version, such as `1.0.4` after `1.0.3`. Never overwrite an existing release tag.
