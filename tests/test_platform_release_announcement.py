...
class PlatformReleaseAnnouncementTests(unittest.TestCase):
    ...
    def test_release_1_0_3_is_loaded_from_project_files(self):
        service = self.namespace['_platform_release_announcement_service']
        payload = service.current_for_user(self.user['id'], 'en')
        self.assertTrue(payload['enabled'])
        self.assertEqual('v1.0.3', payload['id'])
        self.assertEqual('Apervia 1.0.3', payload['version'])
        self.assertEqual('Apervia 1.0.3 brings a more polished and consistent workspace', payload['title'])
        ...
        self.assertEqual('Apervia 1.0.3 带来更精致、更一致的工作空间', chinese['title'])
    ...
    def test_receipt_is_account_scoped_and_removed_with_account(self):
        service = self.namespace['_platform_release_announcement_service']
        service.acknowledge(self.user['id'], 'v1.0.3')
    ...
    def test_release_celebration_has_reduced_motion_fallback(self):
        frontend = (ROOT / 'static/index3/js/index3-account-cloud-lifecycle.js').read_text(encoding='utf-8')
        styles = (ROOT / 'static/index3/css/index3-overrides.css').read_text(encoding='utf-8')
        self.assertIn('closeAppAnnouncementModal({ celebrate:true })', frontend)
        self.assertIn("mask.classList.add('closing')", frontend)
        self.assertIn("mask.classList.add('celebrating')", frontend)
        self.assertIn('app-announcement-confetti', frontend)
        self.assertIn("matchMedia?.('(prefers-reduced-motion: reduce)')", frontend)
        self.assertIn('@media (prefers-reduced-motion:reduce)', styles)
        self.assertIn('@keyframes app-announcement-confetti', styles)
        self.assertIn('@keyframes app-announcement-success', styles)
