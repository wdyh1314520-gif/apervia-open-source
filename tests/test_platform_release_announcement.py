...
class PlatformReleaseAnnouncementTests(unittest.TestCase):
    ...
    def test_release_1_0_0_is_loaded_from_project_files(self):
        service = self.namespace['_platform_release_announcement_service']
        payload = service.current_for_user(self.user['id'], 'en')
        self.assertTrue(payload['enabled'])
        self.assertEqual('v1.0.0', payload['id'])
        self.assertEqual('Apervia 1.0.0', payload['version'])
        self.assertEqual('Apervia 1.0.0 is now available', payload['title'])
        ...
        self.assertEqual('Apervia 1.0.0 正式发布', chinese['title'])
    ...
    def test_receipt_is_account_scoped_and_removed_with_account(self):
        service = self.namespace['_platform_release_announcement_service']
        service.acknowledge(self.user['id'], 'v1.0.0')
